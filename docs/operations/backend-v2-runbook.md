...
脚本不信任 `--environment` 自报标签，而是用 `CLOUDBASE_PROD_ENV_ID/CLOUDBASE_NONPROD_ENV_ID` 判定真实目标；未知或错配 ID 拒绝，生产 ID 必须二次确认。部署前会逐项检查 `cloudbaserc.json` 所需本地绑定非空、pepper/HMAC 最小长度、UUID/整数/布尔安全值，并强制 `TRUST_PROXY_HEADERS=true`、`CSRF_REQUIRED=true`。`SESSION_COOKIE_DOMAIN` 是唯一可选绑定：留空表示 host-only，回读时允许该键缺失或值为空；显式配置时必须原值回读。脚本在 `cloudbase/` cwd 使用官方 `tcb fn deploy --force --yes` 与 `tcb fn detail --env-id`，逐个按结构化字段精确检查 HTTP 类型、gateway/handler、触发器精确集合、env key 精确集合，以及回读的 `TRUST_PROXY_HEADERS=true`；缺少、多出或值错误都失败。无法解析为 JSON 或严格 `字段: 值` 输出时一律失败，不用任意文本包含关系放行。输出仅在进程内校验，不回显可能包含的配置值。不要重新部署旧邀请/旧账号函数。
