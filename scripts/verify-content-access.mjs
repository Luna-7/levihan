...
const ROUTINES = [
  ['get_current_age_policy', ''], ['set_age_consent', 'uuid,text,text'], ['revoke_age_consent', 'uuid,text'],
  ['authorize_work_access', 'uuid,uuid,text'], ['finalize_work_access', 'uuid,uuid,uuid'],
  ['get_public_restricted_access_id', 'text'],
];
...
export function validateContentAccess({ migration, rollback, access, accessRollback, cosSource, serviceSource, swSource }) {
  const failures = [];
  ...
  const consent = routine(sql, 'set_age_consent'); const authorize = routine(sql, 'authorize_work_access'); const finalize = routine(sql, 'finalize_work_access'); const publicWork = routine(sql, 'get_public_work');
  ...
  add(failures, /asset\.storage_zone\s*=\s*'private'/i.test(authorize) && /asset\.access_level\s*=\s*'private'/i.test(authorize) && /asset\.status\s*=\s*'active'/i.test(authorize), 'authorization must select only active private/private assets');
  add(failures, /starts_with\s*\(\s*asset\.object_key\s*,\s*'protected\/works\/'\s*\|\|\s*work\.id::text\s*\|\|\s*'\/'\s*\)/i.test(authorize), 'authorization must enforce work-bound protected prefix');
  ...
  add(failures, /REVOKE\s+INSERT\s*,\s*UPDATE\s*,\s*DELETE\s+ON\s+TABLE\s+public\.age_consents/i.test(grants) && /REVOKE\s+ALL\s+ON\s+TABLE\s+public\.content_access_authorizations/i.test(grants), 'runtime direct authorization DML must be revoked');
  add(failures, !/GRANT\s+(?:SELECT|INSERT|UPDATE|DELETE)[\s\S]*content_access_authorizations/i.test(grants), 'runtime cannot receive direct authorization table grants');
  ...
  add(failures, /expiresInSeconds\s*!==\s*300/i.test(signGet) && /Bucket:\s*privateBucket/i.test(signGet) && /Method:\s*'GET'/i.test(signGet) && /protected\\\/works/i.test(signGet)
    && /parsed\.hostname\s*!==\s*`\$\{privateBucket\}\.cos\.\$\{region\}\.myqcloud\.com`/i.test(signGet) && /q-sign-algorithm/i.test(signGet), 'COS GET signing must be five-minute private protected non-website access');
