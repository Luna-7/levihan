...
const exposed = [
  ...
  ['create_work_comment_v2', 'uuid,uuid,text,text,uuid,text'],
...
export function validateInteractions({ migration, rollback, access, accessRollback, routes, runtime, archive, restrictedPage }) {
  ...
  add(failures, /REVOKE\s+ALL\s+ON\s+TABLE\s+public\.work_likes\s*,\s*public\.favorites\s*,\s*public\.comments\s*,\s*public\.reading_progress\s*,\s*public\.reports/i.test(grants), 'runtime direct interaction DML revoke required');
  add(failures, !/GRANT\s+(?:SELECT|INSERT|UPDATE|DELETE|ALL)[\s\S]*?ON\s+TABLE\s+public\.(?:work_likes|favorites|comments|reading_progress|reports)/i.test(grants), 'runtime direct interaction grants forbidden');
