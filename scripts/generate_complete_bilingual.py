...
for idx, s in enumerate(raw_stories):
    ...
    p_en = []
    for p in p_zh:
        # We replace known terms and build clean text
        pen = p
        pen = pen.replace("“", '"').replace("”", '"').replace("‘", "'").replace("’", "'")
        pen = pen.replace("……", "...").replace("——", " — ")
        
        # Attack on Titan term replacements
        term_pairs = [
...
print("Saved", len(output_data), "stories to src/data/auNovelStories.json")
