...
const CREDENTIAL_LITERAL = /(?:^|[^A-Za-z0-9_$])["'` ]?(?:admin[_-]?password|adminPassword|password|passwd|secret[_-]?key|secretKey|secret|api[_-]?key|apiKey|access[_-]?key[_-]?id|accessKeyId|aws_access_key_id|aws_secret_access_key|private[_-]?key|privateKey|token)["'` ]?\s*[:=]\s*(?:"([^"\r\n]*)"|'([^'\r\n]*)'|`([^`\r\n]*)`)/gi;
...
const TOKEN_SHAPED = /\b(?:Bearer\s+[A-Za-z0-9._~-]{16,}|sk-[A-Za-z0-9]{16,}|gh[pousr]_[A-Za-z0-9]{16,})/g;
