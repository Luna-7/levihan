...
function createApi({ config, router = createRouter(), requestId = crypto.randomUUID, logger = { info() {}, error() {} }, rateLimiter, idempotencyStore, actorResolver = async () => null } = {}) {
  ...
  async function handle(event = {}) {
    ...
    try {
      ...
      if (idempotency.mode === 'domain') {
        const scopeActorHash = actorScopeHash(actorId, clientIp, config.sessionHashPepper);
        context.idempotencyRequestHash = digest({ method, actualPath: normalizedActualPath(relativePath), params: canonicalValue(route.params), query: canonicalQuery(query), routeTemplate: route.path, actorScopeHash: scopeActorHash, body: body === undefined ? null : body });
      }
