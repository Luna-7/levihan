...
function createRuntime({ env = process.env, cloudbase, actorResolver = async () => null } = {}) {
  ...
  return createApi({ config, rateLimiter, idempotencyStore: createCloudBaseIdempotencyStore({ rdb }), actorResolver, logger: createLogger({ actorPepper: config.sessionHashPepper }) });
}
