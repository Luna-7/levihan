...
function parseConfig(env = process.env) {
  ...
  for (const value of [env.SESSION_COOKIE_NAME || 'lv_session', env.CSRF_COOKIE_NAME || 'lv_csrf', env.MIGRATION_COOKIE_NAME || 'lv_migrate']) {
  ...
  const legacyMigrationEnabled = env.LEGACY_MIGRATION_ENABLED === 'true';
  const migrationHashPepper = legacyMigrationEnabled ? required(env, 'MIGRATION_HASH_PEPPER') : undefined;
  if (migrationHashPepper && migrationHashPepper.length < 32) throw new Error('MIGRATION_HASH_PEPPER must be at least 32 characters');
  const trustedProxyHeaders = env.TRUST_PROXY_HEADERS === 'true';
  const proxyHmacSecret = trustedProxyHeaders ? required(env, 'API_PROXY_HMAC_SECRET') : undefined;
  if (proxyHmacSecret && proxyHmacSecret.length < 32) throw new Error('API_PROXY_HMAC_SECRET must be at least 32 characters');
  return {
    environment,
    allowedOrigins,
    sessionCookieName: env.SESSION_COOKIE_NAME || 'lv_session',
    csrfCookieName: env.CSRF_COOKIE_NAME || 'lv_csrf',
    migrationCookieName: env.MIGRATION_COOKIE_NAME || 'lv_migrate',
    cloudbaseApiKey,
    cosPublicBucket,
    cosPrivateBucket,
    cosRegion,
