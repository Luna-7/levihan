...
describe('deployed upload cleanup timer worker', () => {
  ...
  it('accepts Tencent Timer events and invokes only cleanup', async () => {
    const cleanupStalePromotions = vi.fn().mockResolvedValue({ claimed: 2, failed: 0, reportingFailed: 0 });
    ...
    await expect(worker({ Type: 'Timer', TriggerName: 'upload-cleanup-every-10m', Time: '2030-01-01T00:00:00.000Z', Message: 'cleanup' })).resolves.toEqual({ claimed: 2, failed: 0, reportingFailed: 0 });
