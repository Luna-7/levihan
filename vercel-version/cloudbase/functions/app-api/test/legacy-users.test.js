...
describe('one-time legacy credential setup', () => {
  ...
  it('binds a valid migration credential to a short-lived browser claim session', async () => {
    ...
    const service = createLegacyUsersService({ repository, passwordHasher: {}, migrationPepper: 'm'.repeat(32), authPepper: 'a'.repeat(32), opaqueToken: () => 'c'.repeat(43) });
  ...
  it('atomically installs the already-saved recovery secret and returns no irreplaceable response secret', async () => {
    ...
    const service = createLegacyUsersService({ repository, passwordHasher, migrationPepper: 'm'.repeat(32), authPepper: 'a'.repeat(32), opaqueToken: () => 'b'.repeat(43), recoveryCode: () => 'ABCD-EFGH-JKLM-NPQR' });
    ...
    expect(repository.consumeCredential).toHaveBeenCalledWith(expect.objectContaining({ claimSessionHash: expect.stringMatching(/^[a-f0-9]{64}$/), prepareNonceHash: expect.stringMatching(/^[a-f0-9]{64}$/), newPasswordHash: '$argon2id$v=19$secure', recoveryCodeHash: expect.stringMatching(/^[a-f0-9]{64}$/) }));
  ...
  it('returns the one-time recovery secret before consuming the account credential', async () => {
    ...
    const service = createLegacyUsersService({ repository, passwordHasher: {}, migrationPepper: 'm'.repeat(32), authPepper: 'a'.repeat(32), opaqueToken: () => 'd'.repeat(43), recoveryCode: () => 'ABCD-EFGH-JKLM-NPQR' });
  ...
  it.each([
    ...
    const service = createLegacyUsersService({ repository: {}, passwordHasher: { hash: vi.fn() }, migrationPepper: 'm'.repeat(32), authPepper: 'a'.repeat(32) });
  ...
  it('fails closed when the compatibility feature is disabled', async () => {
    const service = createLegacyUsersService({ repository: {}, passwordHasher: { hash: vi.fn() }, migrationPepper: 'm'.repeat(32), authPepper: 'a'.repeat(32), enabled: false });
