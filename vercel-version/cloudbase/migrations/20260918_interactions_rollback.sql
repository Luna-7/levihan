...
DO $$ BEGIN
  IF EXISTS(SELECT 1 FROM public.moderation_actions WHERE action IN ('review','resolve'))
    ...
    OR EXISTS(SELECT 1 FROM public.reports WHERE note<>'')
    OR EXISTS(SELECT 1 FROM public.comment_idempotency)
...
DROP FUNCTION IF EXISTS public.create_work_comment_v2(uuid,uuid,text,text,uuid,text,text);
...
DROP TABLE IF EXISTS public.comment_idempotency;
ALTER TABLE public.works DROP CONSTRAINT IF EXISTS works_slug_not_uuid;
