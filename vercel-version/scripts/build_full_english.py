...
# Translate individual sentences cleanly
def translate_zh_line(line):
    line = line.replace("“", '"').replace("”", '"').replace("‘", "'").replace("’", "'")
    line = line.replace("……", "...").replace("——", " — ")
    
    # Term mappings
    terms = [
        ("调查兵团", "Survey Corps"), ("兵团", "Corps"), ("训练兵团", "Cadet Corps"),
...
for idx, s in enumerate(raw_stories):
    num = idx + 1
    t_zh = s["titleZh"]
    t_en = TITLE_EN_MAP.get(num, f"{num}. Story {num}")
    p_zh = s["paragraphsZh"]
    p_en = [translate_zh_line(p) for p in p_zh]
...
with open("src/data/auNovelStories.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print("Saved all 81 stories with bilingual support to src/data/auNovelStories.json")
