...
describe('hard-coded secret scanner', () => {
  ...
  it('redacts canary values in CLI errors and only emits path, line, rule, and marker', () => {
    ...
    try {
      const verification = spawnSync(process.execPath, [resolve(packageRoot, 'scripts/verify-no-secrets.mjs'), temporaryDirectory], {
        cwd: packageRoot,
        encoding: 'utf8',
      });
  ...
  it('does not scan its own explicit fixture', () => {
    const fixturePath = resolve(packageRoot, 'scripts/verify-no-secrets.fixture.ts');
