...
describe('content access schema verifier', () => {
  ...
  it.each([
    ...
    ['COS complete V5 fields', (value: ReturnType<typeof files>) => { value.cosSource = value.cosSource.replace("'q-header-list', 'q-url-param-list',", ''); }],
    ['COS signed query scope', (value: ReturnType<typeof files>) => { value.cosSource = value.cosSource.replace('signedQueryNames.some((name, index) => name !== actualQueryNames[index])', 'false'); }],
    ['COS signature-field shadowing', (value: ReturnType<typeof files>) => { value.cosSource = value.cosSource.replace('COS_V5_RESERVED_CANONICAL_FIELDS.has(canonical)', 'false'); }],
    ['COS remaining validity', (value: ReturnType<typeof files>) => { value.cosSource = value.cosSource.replace('end > nowSeconds + 300', 'end > nowSeconds + 360'); }],
    ['COS future start', (value: ReturnType<typeof files>) => { value.cosSource = value.cosSource.replace('start > nowSeconds', 'start > nowSeconds + 60'); }],
