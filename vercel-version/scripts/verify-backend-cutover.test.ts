...
describe('backend v2 cutover schema', () => {
  ...
  it.each([
    ...
    ['public revoke', 'migration', (sql: string) => sql.replace('public.consume_legacy_migration_credential(text,text,text,text,text,text)', 'public.removed_consume(text)')],
    ...
    ['runtime rollback signature', 'runtimeRollback', (sql: string) => sql.replace('(text,text,text,text,text,text)', '(text,text,text,text,text,text,text)')],
