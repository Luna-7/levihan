...
describe('CloudBase deployment guard', () => {
  ...
  const listed = (names: string[]) => ({ stdout: JSON.stringify({ functions: names.map((name) => ({ name })) }) });
  ...
  it('pins and reads back every deployment against the explicit environment id', async () => {
    let listCall = 0;
    const runner = vi.fn(async (args: string[], options: { cwd: string; env: Record<string, string> }) => {
      if (args[1] === 'list') return { ...listed(listCall++ === 0 ? [] : ['app-api','snapshot-worker','upload-cleanup-worker']), options };
      return { stdout: args[1] === 'detail' ? JSON.stringify(detail(args[2])) : '', options };
    });
    ...
    expect(runner.mock.calls[0][0]).toEqual(['fn','list','--env-id','staging-123','--json']);
    expect(runner.mock.calls[1][0].slice(0, 3)).toEqual(['fn','deploy','app-api']);
    expect(runner.mock.calls[1][0]).toEqual(expect.arrayContaining(['--force', '--yes']));
  });

  it('refuses legacy or unexpected remote functions before deploying anything', async () => {
    for (const name of ['registerWithPassword', 'loginWithPassword', 'generateInviteCode', 'admin-upload', 'unknown-writer']) {
      const runner = vi.fn(async (args: string[]) => args[1] === 'list' ? listed([name]) : { stdout: '' });
      await expect(deployCloudBase(parseDeployOptions(['--apply=DEPLOY_BACKEND_V2','--environment=nonprod','--env-id=staging-123'], env), runner, deployEnv)).rejects.toThrow(/remote function inventory/);
      expect(runner).toHaveBeenCalledTimes(1);
    }
  });

  it('fails closed when the post-deploy inventory is not the exact v2 allowlist', async () => {
    let listCall = 0;
    const runner = vi.fn(async (args: string[]) => {
      if (args[1] === 'list') return listed(listCall++ === 0 ? [] : ['app-api','snapshot-worker','upload-cleanup-worker','submitComment']);
      if (args[1] === 'detail') return { stdout: JSON.stringify(detail(args[2])) };
      return { stdout: '' };
    });
    await expect(deployCloudBase(parseDeployOptions(['--apply=DEPLOY_BACKEND_V2','--environment=nonprod','--env-id=staging-123'], env), runner, deployEnv)).rejects.toThrow(/remote function inventory/);
  });
  ...
  it('rejects read-back text where expected values only appear in an unrelated field', async () => {
    const runner = vi.fn(async (args: string[]) => args[1] === 'list' ? listed([]) : args[1] === 'deploy' ? { stdout: '' } : { stdout: JSON.stringify({
  ...
  it('rejects missing or false trusted proxy read-back and any extra trigger', async () => {
    for (const override of [
      ...
      const runner = vi.fn(async (args: string[]) => args[1] === 'list' ? listed([]) : ({ stdout: JSON.stringify(detail(args[2], args[2] === 'app-api' ? override : {})) }));
      await expect(deployCloudBase(parseDeployOptions(['--apply=DEPLOY_BACKEND_V2','--environment=nonprod','--env-id=staging-123'], env), runner, deployEnv)).rejects.toThrow(/read-back/);
    }
  });
  ...
  it('accepts a host-only cookie deployment when SESSION_COOKIE_DOMAIN is absent from read-back', async () => {
    let listCall = 0;
    const runner = vi.fn(async (args: string[]) => { if (args[1] === 'list') return listed(listCall++ === 0 ? [] : ['app-api','snapshot-worker','upload-cleanup-worker']); const environmentVariables = functionEnv(args[2]); delete environmentVariables.SESSION_COOKIE_DOMAIN; return { stdout: JSON.stringify(detail(args[2], { environmentVariables })) }; });
