...
const IGNORED_FILE_SUFFIXES = ['scripts/verify-no-secrets.fixture.ts'];
...
function shouldIgnore(relativePath, isDirectory) {
  if (IGNORED_FILE_SUFFIXES.some((suffix) => relativePath === suffix || relativePath.endsWith(`/${suffix}`))) return true;
