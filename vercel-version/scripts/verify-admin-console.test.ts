...
describe('admin console verifier',()=>{it('accepts controlled console',()=>expect(validateAdminConsole(files())).toEqual([]));it.each([
 ...
 ['promotion reauth',(v:ReturnType<typeof files>)=>{v.migration=v.migration.replace("reauthenticated_at>=clock_timestamp()-interval '5 minutes'",'true');}],
 ['settings whitelist',(v:ReturnType<typeof files>)=>{v.migration=v.migration.replace("p_key NOT IN('announcement','adult_content_policy','feature_flags')",'false');}],
 ['settings seed',(v:ReturnType<typeof files>)=>{v.migration=v.migration.replace("VALUES('announcement'", "VALUES('missing_announcement'");}],
 ['health migration signal',(v:ReturnType<typeof files>)=>{v.migration=v.migration.replace('backend_v2_migration_runs','missing_migration_runs');}],
