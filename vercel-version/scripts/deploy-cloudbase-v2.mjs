...
const LEGACY_FUNCTIONS = new Set(['registerWithPassword','loginWithPassword','consumeInviteCode','generateInviteCode','validateRegistrationInvite','verifyInviteCode','createUserProfile','getUserAccount','updateNickname','admin-upload','submitComment','deleteComment','submitGameScore','getGameLeaderboard']);
...
function parseFunctionInventory(text) {
  let value;
  try { value = JSON.parse(String(text).replace(/\u001b\[[0-9;]*m/g, '').trim()); } catch { throw new Error('remote function inventory is not structured JSON'); }
  const entries = Array.isArray(value) ? value : value?.functions ?? value?.data ?? value?.list;
  if (!Array.isArray(entries)) throw new Error('remote function inventory is missing');
  const names = entries.map((entry) => typeof entry === 'string' ? entry : entry?.name ?? entry?.functionName);
  if (names.some((name) => typeof name !== 'string' || !name.trim()) || new Set(names).size !== names.length) throw new Error('remote function inventory is invalid');
  return names;
}
function validateRemoteFunctionInventory(text, phase, envId) {
  const names = parseFunctionInventory(text);
  const forbidden = names.filter((name) => LEGACY_FUNCTIONS.has(name));
  const unexpected = names.filter((name) => !FUNCTIONS.includes(name));
  const missing = phase === 'post-deploy' ? FUNCTIONS.filter((name) => !names.includes(name)) : [];
  if (forbidden.length || unexpected.length || missing.length) {
    const targets = [...new Set([...forbidden, ...unexpected])].sort();
    const remediation = targets.length ? ` Disable their HTTP/timer triggers, then run: ${targets.map((name) => `tcb fn delete ${name} --env-id ${envId} --yes`).join(' ; ')}` : '';
    throw new Error(`remote function inventory rejected during ${phase}.${remediation}`);
  }
  return names;
}
...
export async function deployCloudBase(options, runner = async (args, commandOptions) => {
  ...
  const before = await runner(['fn', 'list', '--env-id', options.envId, '--json'], commandOptions);
  validateRemoteFunctionInventory(String(before?.stdout || before || ''), 'pre-deploy', options.envId);
  ...
  const after = await runner(['fn', 'list', '--env-id', options.envId, '--json'], commandOptions);
  validateRemoteFunctionInventory(String(after?.stdout || after || ''), 'post-deploy', options.envId);
