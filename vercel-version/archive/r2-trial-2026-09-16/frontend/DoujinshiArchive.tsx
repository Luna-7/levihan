...
export const DoujinshiArchive: React.FC<Props> = ({ onShowToast, onGoToResources }) => {
  ...
  const handleOpenBookReader = async (book: DoujinBookItem) => {
    ...
    if (book.storageProvider === 'r2' || book.sensitiveLevel === 'mild-r18') {
      onShowToast('该作品已安全存入私有敏感作品库，阅读权限将在下一阶段开放 🔒');
      return;
    }
    setReadingBook(book);
  ...
  if (readingBook) {
    ...
    return (
      <div id="seamless-doujin-reader" className="relative w-full select-text pb-12" style={{ touchAction: 'pan-x pan-y pinch-zoom' }}>
        {/* 顶部快捷导航 */}
            ...
            >
              <div className="space-y-2">
                ...
                <div className="relative w-full aspect-[2/3] bg-[#FAF5E8] border border-[#E0D5BE] rounded-xs overflow-hidden group-hover:border-[#1E4334] flex items-center justify-center transition-colors">
                  {book.storageProvider === 'r2' || book.sensitiveLevel === 'mild-r18' ? (
                    <div className="flex flex-col p-3 text-center w-full h-full items-center justify-center bg-[#2B211B] text-[#F4DFC0]">
                      <div className="text-2xl">🔒</div>
                      <div className="font-pixel text-[10px] mt-2 font-bold">微量 R18</div>
                      <div className="text-[9px] text-[#D8B98A] mt-1">私有存储 · 暂未开放阅读</div>
                    </div>
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="px-3 py-1.5 bg-[#1E4334] text-[#F9E79F] font-pixel text-xs rounded-xs shadow-lg flex items-center gap-1">
                      <span>{book.storageProvider === 'r2' ? '🔒 阅读权限暂未开放' : '📖 点击阅读长图'}</span>
                      <span>→</span>
                    </span>
                  </div>
                </div>
