...
function normalizeBook(raw) {
  ...
  if (raw.sensitiveLevel === 'mild-r18') {
    book.sensitiveLevel = 'mild-r18';
    book.storageProvider = 'r2';
    book.accessPolicy = 'login-age-confirmation';
  }
...
async function handle(action, payload) {
  switch (action) {
    case 'verifyToken': {
      if (!verifyToken(String(payload.token || ''))) throw httpError('管理员令牌无效或已过期', 401);
