...
(function (root) {
  ...
  var runtime = {
    ...
    renderPdfPages: null,
    /**
     * 带密码 PDF 处置器，签名 (file) => Promise<File>。
     * 由管理台注入（复用 index.html 的 pdf.js）。返回真正要拿去加密上传的 File：
     *   - 无密码 / 内层口令 = 阅读端已知的工具箱口令 且 无超长条漫页 → 原样返回（存原始字节）；
     *   - 有超长条漫页 → 重渲染并纵向切片成正常比例段（否则阅读端画布/压缩档会把宽度压扁）；
     *   - 自定义口令 → 解锁并导出无密码副本（阅读端只认已知口令，不去掉就是死文件）；
     *   - 密码错误/取消 → reject 中止上传。没注入时跳过（保持原字节直通行为）。
     */
    unlockPdf: null,
  };
